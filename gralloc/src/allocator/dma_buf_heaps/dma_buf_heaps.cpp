...
int allocator_allocate(const buffer_descriptor_t *descriptor, private_handle_t **out_handle)
{
	...
	int fd = allocator->Alloc(heap_name, descriptor->size);
	if (fd < 0)
	{
		if (heap == dma_buf_heap::system || heap == dma_buf_heap::protected_memory)
		{
		...
		else
		{
			...
			fd = allocator->Alloc(get_dma_buf_heap_name(dma_buf_heap::system), descriptor->size);
			if (fd < 0)
			{
	...
	*out_handle = make_private_handle(
	    0, descriptor->size,
	    descriptor->consumer_usage, descriptor->producer_usage, fd, descriptor->hal_format,
	...
	/*
	 * Note: this mapping occurs in the allocator process. When handles are transferred across process boundaries,
	 * mappings are reset. We expect subsequent calls to mali_gralloc_allocator_map from client processes.
	 */
	int ret = allocator_map(*out_handle);
	if (ret != 0)
	{
		allocator_free(*out_handle);
		free(*out_handle);
		return ret;
	}
...
#define GRALLOC_INIT_AFBC 0
	...
	if (GRALLOC_INIT_AFBC && is_format_afbc(descriptor->alloc_format))
	{
		...
		allocator_sync_end(*out_handle, true, true);
	}
...
void allocator_unmap(private_handle_t *handle)
{
	...
	handle->base = nullptr;
	handle->cpu_read = handle->cpu_write = false;
}
